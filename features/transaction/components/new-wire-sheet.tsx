...
import { useCreateWireTransfer } from "@/features/transaction/api";
import {
  ApiValues,
  WireForm,
} from "@/features/transaction/components/wire-form";
import { useNewWire } from "@/features/transaction/hooks";
import { AccountBank, Categorie } from "@prisma/client";

export const NewWireSheet = () => {
  const { isOpen, onClose } = useNewWire();
  const mutation = useCreateWireTransfer();
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent className="space-y-4">
        <SheetHeader>
          <SheetTitle>New Wire Transfer</SheetTitle>
          <SheetDescription>Add a new wire transfer</SheetDescription>
        </SheetHeader>
        {isLoading ? (
